...
double ZZH6CFit::JetEnergyResolution(double E)
{
...
ZZH6CFit::ZZH6CFit() : Processor("ZZH6CFit") {
  
  // modify processor description
  _description = "ZZH6CFit does a kinematic fit on 2 lepton + 4 jet events with 4C from 4-momentum conservation + 2C from MH = 125 GeV and MZ = 90 GeV" ;
...
void ZZH6CFit::processRunHeader( LCRunHeader* ) { 
  _nRun++ ;
} 

void ZZH6CFit::processEvent( LCEvent * evt ) { 
    ...
    streamlog_out(DEBUG)  << "*l1" << *l1  << "*l2" << *l2  << std::endl ;
	 ...
	 hRecHMassNoFitAll->fill( startmass1 ) ;
	   ...
	   streamlog_out(DEBUG)  << "final four-vector of ISR photon: " << *(photon) << std::endl ;
         ...
         streamlog_out(DEBUG)  << "final mass of H 1: " << w1.getMass(1) << std::endl ;
	 streamlog_out(DEBUG)  << "final mass of H 2: " << w2.getMass(1) << std::endl ;
         ...
         if (fitter.getError() == 0) {
           ...
           hRecHMassAll->fill( w1.getMass(1)) ;
	   hRecHMassAll->fill( w2.getMass(1)) ;
#endif
	   //           if (prob > bestprob && w.getMass(1) > 50 && w.getMass(1) < 110) {
	     ...
	     bestchi2 = chi2;
             bestprob = prob;
             bestnit  = nit;
             bestmass1 = w1.getMass(1);
             bestmass2 = w2.getMass(1);
